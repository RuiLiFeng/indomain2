...
def parse_args():
  ...
  parser.add_argument('--boundary_path_cond', type=str,
                      help='Path to the boundary for semantic manipulation.')
...
def main():
  ...
  boundary_path_cond = args.boundary_path_cond
  c_boundary_name = 'foo'
  if os.path.exists(args.boundary_path_cond):
      c_boundary_name = os.path.splitext(os.path.basename(boundary_path_cond))[0]
  output_dir = args.output_dir or 'results/manipulation'
  job_name = f'{boundary_name}_cond_on_{c_boundary_name}_{image_dir_name}'
  ...
  if os.path.exists(boundary_path_cond):
    c_boundary = np.load(boundary_path_cond, allow_pickle=True)[()]
  ...
  if os.path.exists(boundary_path_cond):
    codes = manipulate(latent_codes=codes[:, args.ind],
                       boundary=c_boundary,
                       start_distance=args.start_distance,
                       end_distance=args.end_distance,
                       step=step,
