...
def Encoder(
    ...
    with tf.variable_scope('Latent_out'):
        hidden = apply_bias(dense(net, fmaps=dlatent_size * num_layers, use_wscale=False))
        latent_w = sync_batch_norm(hidden, is_training=is_training, num_dev=num_gpus)
    ...
    with tf.variable_scope('Latent_radius'):
        ...
        latent_radius = apply_bias(dense(hidden, fmaps=1 * num_layers, use_wscale=True))
